...
#' @export
get_tract_demographics = function(state_name, county_fips = NULL, endyear=2015, span=5)
{  
...
#' Get a handful of economic variables on Census Tracts in a State from the US Census Bureau as a data.frame.
#' 
#' The data comes from the American Community Survey (ACS). The variables are: total population, percent White 
#' not Hispanic, Percent Black or African American not Hispanic, percent Asian not Hispanic,
#' percent Hispanic all races, per-capita income, median rent and median age.
...
#' @importFrom acs acs.fetch
#' @export
get_tract_economics = function(state_name, county_fips = NULL, endyear=2015, span=5)
{  
  state_fips = get_state_fips_from_name(state_name)
  ...
  # poverty
  poverty.data = acs::acs.fetch(geography    = tracts, 
                             table.number = "B17001", 
                             col.names    = "pretty", 
                             endyear      = endyear, 
                             span         = span)
  ...
  # dummy to get proper regions
  poverty.dummy.df = convert_acs_obj_to_df("tract", poverty.data, 1, FALSE)
  ...
  df_economics = merge(df_race        , df_income, all.x=TRUE)
  df_economics = merge(df_economics, df_poverty  , all.x=TRUE)		
  df_economics = merge(df_economics, df_rent  , all.x=TRUE)  
  df_economics = merge(df_economics, df_age   , all.x=TRUE)
